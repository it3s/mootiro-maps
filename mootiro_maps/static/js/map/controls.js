...
define(function(require) {
  ...
  PerimeterSelector = (function(_super) {
    ...
    PerimeterSelector.prototype.selected = function(latLng) {
      ...
      if (typeof this.callback === "function") this.callback(latLng, this.circle);
  ...
  StreetView = (function(_super) {
    ...
    StreetView.prototype.createObject = function() {
      ...
      if ((_ref = this.map) != null) _ref.googleMap.setStreetView(this.streetView);
  ...
  FeatureTypeFilter = (function(_super) {

    __extends(FeatureTypeFilter, _super);

    function FeatureTypeFilter() {
      FeatureTypeFilter.__super__.constructor.apply(this, arguments);
    }

    FeatureTypeFilter.prototype.hooks = {
      'before_feature_setVisible': 'beforeFeatureSetVisibleHook'
    };
    ...
    FeatureTypeFilter.prototype.beforeFeatureSetVisibleHook = function(feature, visible) {
      var visible_, _ref;
      visible_ = visible;
      if (visible && (_ref = feature.featureType.type, __indexOf.call(this.disabled, _ref) >= 0)) {
        visible_ = false;
      }
      return [feature, visible_];
    };

    FeatureTypeFilter.prototype.handleMapEvents = function() {
      var _this = this;
      this.map.subscribe('hide_features_by_type', function(type, categories, strict) {
        if (__indexOf.call(_this.disabled, type) < 0) {
          return _this.disabled.push(type);
        }
      });
  ...
  LayersFilter = (function(_super) {

    __extends(LayersFilter, _super);

    function LayersFilter() {
      LayersFilter.__super__.constructor.apply(this, arguments);
    }

    LayersFilter.prototype.hooks = {
      'before_feature_setVisible': 'beforeFeatureSetVisibleHook'
    };
    ...
    LayersFilter.prototype.beforeFeatureSetVisibleHook = function(feature, visible) {
      var visible_;
      visible_ = visible;
      if (visible && !this.map.getLayers().shouldFeatureBeVisible(feature)) {
        visible_ = false;
      }
      return [feature, visible_];
    };

    LayersFilter.prototype.handleMapEvents = function() {
      var _this = this;
      this.map.subscribe('show_layer', function(layer) {
        if (__indexOf.call(_this.disabled, layer) < 0) {
          return _this.disabled.push(layer);
        }
      });
      return this.map.subscribe('hide_layer', function(layer) {
        var index;
        if (__indexOf.call(_this.disabled, layer) >= 0) {
          index = _.indexOf(_this.disabled, layer);
          return _this.disabled.splice(index, 1);
        }
      });
    };
